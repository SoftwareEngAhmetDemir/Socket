...
app.get('/gettarih/:ilanid',(req,res)=>{
  ...
  var url =
    ...
    MongoClient.connect(url, function(err, db) {
      if (err) throw err;
      var dbo = db.db("test");
      var query = { ilanod: req.params.ilanid };
      dbo.collection("estate").find(query).toArray(function(err, result) {
        if (err) throw err;
        console.log(result);
        db.close();
      });
...
app.get('/update/:ilanid/:userid/:username/:fiyat',(req,res)=>{
  ...
  var url ="mongodb+srv://murat:murat3838@cluster0-2yepv.gcp.mongodb.net/test?authSource=admin&replicaSet=Cluster0-shard-0&w=majority&readPreference=primary&appname=MongoDB%20Compass&retryWrites=true&ssl=true";
...
// ///////////////////////
app.get("/insertfiyat/:ilanid/:userid/:username/:fiyat", async (req, res) => {
  var mresult = [];
